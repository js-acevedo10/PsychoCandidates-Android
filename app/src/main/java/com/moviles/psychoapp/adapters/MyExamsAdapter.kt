...
class MyExamsAdapter(val items: List<BriefExam>, val layout: Int, val listener: (BriefExam) -> Unit) : RecyclerView.Adapter<MyExamsAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItem(item: BriefExam, listener: (BriefExam) -> Unit) = with(itemView) {
